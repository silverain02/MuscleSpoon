module.exports = {
  HTML:function(list){
    ...
    <body>
      ...
      <script>
        ...
        // 체크한 목표 저장
        function Checkbox(){ //cheak 되면 데이터베이스 변경
          if(check === 'Y'){}
        }
  ...
  }, list:function(exerData){
    var list = '<ul>';
    var i = 0;
    while(i < exerData.length){
      if(exerData[i].userId === queryData.userId){
        list = list + 
        `<li><input type="checkbox"> ${exerData[i].part} ${exerData[i].exerName} ${exerData[i].setNum} 
